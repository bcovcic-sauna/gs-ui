...
- 🎨 **Beautiful & Accessible**: Built on Radix UI primitives for full accessibility
- 🎯 **TypeScript**: Fully typed components with excellent IDE support
- 🎨 **Tailwind CSS**: Styled with Tailwind for easy customization
- 📦 **Source Files Included**: Components are copied to your project for full control
- 🔧 **Tree-shakeable**: Import only what you need
- 🚀 **Modern**: Built for React 18+ and 19+
...
function main() {
  console.log(`📦 Copying UI component sources from ${PACKAGE_NAME}...`);

  if (!fs.existsSync(PACKAGE_SRC)) {
    console.warn(`⚠️  Warning: ${PACKAGE_NAME}/src not found in node_modules.`);
  ...
  if (fs.existsSync(TARGET_DIR)) {
    console.log(`🗑️  Removing existing components/ui directory...`);
    fs.rmSync(TARGET_DIR, { recursive: true, force: true });
  }

  console.log(`📋 Copying to ${path.relative(process.cwd(), TARGET_DIR)}...`);
  ...
  try {
    ...
    if (fs.existsSync(componentsDir)) {
      ...
      console.log(`   ✓ Copied ${files.length} component files (imports fixed to @/lib/utils)`);
    }
...
- ⚠️ **Do not modify files in `components/ui/`** - They will be overwritten on package updates
- ✅ Components are automatically updated when you run `npm install` after a package update
- ✅ The `index.ts` barrel file is auto-generated - do not edit it manually
